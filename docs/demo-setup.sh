...
# tmux server (socket: lu-demo, own config) — your real tmux server and config
# are never touched.
#
...
#
# HONEST BY DESIGN: the capsule is the plugin's own #{llm_usage} token, rewritten
# by llm-usage.tmux into a non-blocking #(usage.sh) reader. The numbers come from
# the bundled examples/static.sh provider (a fixed contract-v1 payload), so the
# capsule is deterministic and needs no real quota endpoint or API key. Requires
# jq (used by usage.sh to render the provider JSON).
set -u
...
SR="${SR}$(pill '#cba6f7' 'LLM' '#{llm_usage}')"
...
# ── llm-usage config + the token-bearing status-right, then load the plugin so
#    it writes its config and rewrites #{llm_usage} into #(usage.sh). Finally warm
#    the cache with a foreground __sync__ IN THE SERVER CONTEXT (redirected to
#    /dev/null so run-shell never echoes the value into the pane's copy-mode). ──
...
"$TMUX_BIN" -L "$SOCK" run-shell "$PLUGIN/scripts/usage.sh __sync__ >/dev/null 2>&1"
